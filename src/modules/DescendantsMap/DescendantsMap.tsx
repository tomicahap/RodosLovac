...
import { generateDescendantGeoData, PlotType } from './utils/descendantGeoDataParser';
...
export default function DescendantsMap() {
  ...
  useEffect(() => {
    ...
    generateDescendantGeoData(tree, graph, selectedPersonId, plotType, maxGenerations, (p) => {
  ...
  const getGenDescription = (gen: number) => {
    if (gen === 2) return "— djeca";
    if (gen === 3) return "— unuci";
    if (gen === 4) return "— praunuci";
    return `— ${gen - 3}x praunuci`;
  };
  ...
  const createClusterIcon = (cluster: any) => {
    ...
    return L.divIcon({
      html: `<div class="w-8 h-8 bg-green-600 rounded-full border-2 border-white shadow-md flex items-center justify-center text-white text-xs font-bold">${count}</div>`,
  ...
  if (!selectedPersonId) {
    return (
      <div className="flex-1 flex items-center justify-center text-slate-400 font-medium">
        Odaberite osobu za prikaz karte potomaka.
      </div>
    );
  }
        ...
        <div className="bg-white shadow-sm rounded-xl p-2 flex flex-wrap sm:flex-nowrap items-center gap-2 border border-slate-200 w-full">
          ...
          <div className="flex gap-1 bg-slate-50 p-1 rounded-lg shrink-0 border border-slate-100">
            <button 
              ...
              className={`px-4 py-1.5 rounded-md text-sm font-bold transition-colors ${plotType === 'BIRTH' ? 'bg-green-50 text-green-700 border border-green-200 shadow-sm' : 'text-slate-600 hover:text-slate-900 border border-transparent'}`}
            >
              Mjesta rođenja
            </button>
            <button 
              onClick={() => setPlotType('DEATH')}
              className={`px-4 py-1.5 rounded-md text-sm font-bold transition-colors ${plotType === 'DEATH' ? 'bg-green-50 text-green-700 border border-green-200 shadow-sm' : 'text-slate-600 hover:text-slate-900 border border-transparent'}`}
            >
              Mjesta smrti
            </button>
            <button 
              onClick={() => setPlotType('MOVES')}
              className={`px-4 py-1.5 rounded-md text-sm font-bold transition-colors ${plotType === 'MOVES' ? 'bg-green-50 text-green-700 border border-green-200 shadow-sm' : 'text-slate-600 hover:text-slate-900 border border-transparent'}`}
            >
              Životna kretanja
            </button>
          </div>
          ...
          <span className="text-sm text-slate-500 font-medium ml-2 hidden lg:block whitespace-nowrap">
            {plotType === 'BIRTH' && '— gdje su potomci rođeni'}
            {plotType === 'DEATH' && '— gdje su potomci umrli'}
          ...
          <div className="flex gap-1 overflow-x-auto custom-scrollbar no-scrollbar py-1">
            {GENERATIONS.map(gen => (
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-lg text-sm font-bold shrink-0 transition-colors ${maxGenerations === gen ? 'bg-green-500 text-white shadow-sm border border-green-600' : 'bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100'}`}
              >
                {gen}G
              </button>
            ))}
      ...
      <div className="flex-1 px-4 pb-4 min-h-[400px] flex flex-col max-w-7xl mx-auto w-full relative">
            ...
            {plotType === 'MOVES' && flows.map((f, i) => {
              ...
              return (
                <Polyline 
                  key={`flow-${i}`} 
                  positions={coords} 
                  pathOptions={{ color: '#16a34a', weight: 2, opacity: 0.5, dashArray: '4,6' }} 
                />
              );
            })}
                        ...
                        <div className="max-h-40 overflow-y-auto custom-scrollbar pr-2 space-y-2">
                          {group.map((f, idx) => (
                            <div key={idx} className="flex flex-col">
                              <span className="text-sm font-bold text-green-700">{f.properties.fullName}</span>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-4 text-sm px-2 text-slate-500">
          <div className="font-bold text-slate-700">
            {stats.peopleCount} {stats.peopleCount === 1 ? 'potomak' : 'potomaka'} {plotType === 'BIRTH' ? 'rođen' : plotType === 'DEATH' ? 'umro' : 'živio'} na {stats.locationsCount} {stats.locationsCount === 1 ? 'lokaciji' : 'lokacija'}
          </div>
